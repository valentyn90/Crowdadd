...
class Helper extends Model
{
    ...
    public static function getCategoryOrders($category_id)
    {
      return  ServiceUser::with('user','service')
                ->whereHas('service', function (Builder $query) use($category_id) {
    ...
    public static function getServiceAvailableDate($service_id, $status)
    {
        $service =  ServiceUser::where('service_id', $service_id)
            ->where('status',$status)
            ->orderBy('id','desc')
            ->first();
        $date = isset($service) ? Carbon::parse($service->to) : Carbon::now();
