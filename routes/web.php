...
Route::group(
    ...
    function () {
        ...
        // Booking Overview
        Route::get('freelancer/booking-overviews', 'FreelancerBookingOverviewController@index')->name('freelancer.booking-overviews.index');
    }
);
...
//for calender
Route::get('/savedates/{category_id}/{date}', function($category_id,$date){

    if ($date == 'null') {
        $rows = DB::table('mark_offdays')->where('user_id',Auth::user()->id)->where('category_id', $category_id)->count();
        
        if ($rows == 0) {
            DB::table('mark_offdays')->insert(
                 array(
            ...
            DB::table('mark_offdays')->where('user_id',Auth::user()->id)->where('category_id', $category_id)->update(
                 array(
                        'dates'   =>  null
                 )
            ); 
...
// Route::get('/saveavailabledates/{category_id}/{date}', function($category_id,$date){

//     if ($date == 'null') {
//         $rows = DB::table('availabledays')->where('user_id',Auth::user()->id)->where('category_id',$category_id)->count();
//         if ($rows == 0) {
//             DB::table('availabledays')->insert(
//                  array(
//                         'user_id'     =>   Auth::user()->id, 
//                         'category_id' => $category_id,
//                         'dates'   =>   null
...
//             DB::table('availabledays')->where('user_id',Auth::user()->id)->where('category_id',$category_id)->update(
//                  array(
//                         'dates'   =>  null
//                  )
//             ); 
...
Route::get('/getdates/{category}', function($category = null){
    $service_id = App\Service::where('category_id',$category)->first()->id;
    if (!empty($service_id)) {
        $service_user = App\ServiceUser::where('service_id',$service_id)->where('seller_id',Auth::user()->id)->with('service')->get();return $service_user->toArray();;        
    }
});
...
Route::get('/getunavailabledates/{category_id}', function($category_id = null){

    $dates = DB::table('mark_offdays')->where('user_id',Auth::user()->id)->where('category_id', $category_id)->get();
...
// Route::get('/getavailabledates/{category_id}', function($category_id = null){

//     $dates = DB::table('availabledays')->where('user_id',Auth::user()->id)->where('category_id',$category_id)->get();
...
Route::get('/getunavailabledatesbyid/{id}', function($id = null){
    $dates = DB::table('mark_offdays')->where('user_id',$id)->get();
    foreach ($dates as $date) {
        $dates = explode(',', $date->dates);
        return $dates;
    }
...
Route::get('/getfreedates/{category}', function($category = null){
    if (!empty($category)) {
        $service_user = App\ServiceUser::where('service_id',$category)->first();
        $order_dates = $service_user->dates; 
        $order_user = $service_user->seller_id;

        $markoffdates = DB::table('mark_offdays')->where('user_id',$order_user)->where('category_id',$category)->pluck('dates');
        $offdays = $markoffdates[0];  
        // $availabledays =  DB::table('availabledays')->where('user_id',$order_user)->where('category_id',$category)->pluck('dates');

        // $daysavilable =  $availabledays[0];
        // return $offdays;

        return [$order_dates,$offdays];
    }
});
